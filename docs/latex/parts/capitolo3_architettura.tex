...
business e presentazione. La struttura è stata progettata per garantire modularità,
assegnando a ciascun componente responsabilità ben definite. Questa organizzazione facilita
l'estensione del sistema con nuovi algoritmi o funzionalità, mantenendo il codice pulito e
bendocumentato. I componenti isolati risultano inoltre facilmente testabili, migliorando
complessivamente la manutenibilità del progetto.
...
Contiene tutta la logica steganografica, completamente indipendente dall'UI. Ciascun modulo
gestisce un tipo specifico di dato: \texttt{message\_operations.py} per l'embedding e
l'estrazione di stringhe, \texttt{image\_operations.py} per le immagini, e
\texttt{binary\_operations.py} per file binari arbitrari.

\subsection{Utility Layer}

Fornisce servizi trasversali utilizzati da tutti gli algoritmi. Il modulo \texttt{backup.py}
gestisce serializzazione e deserializzazione dei parametri in formato JSON, mentre
\texttt{validator.py} si occupa della validazione di dimensioni, formati e capacità.
Il calcolo delle metriche PSNR e SSIM è centralizzato in \texttt{metrics.py}, le operazioni
binarie e conversioni in \texttt{bit\_operations.py}, e la gestione I/O dei file in
\texttt{file\_utils.py}.
